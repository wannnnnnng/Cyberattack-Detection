...
"""
使用逻辑回归模型进行训练和评估
逻辑回归模型使用了 class_weight 参数来处理类别不平衡问题，
使得模型对少数类样本更加敏感，提高了少数类样本的识别率。
Parameters
----------
...
for i in range(0, nb_prediction):
    ...
    clf = linear_model.LogisticRegression(penalty='l2', C=550, random_state=tab_seed[i], multi_class="auto",
                                          class_weight={0: 0.044, 1: 1 - 0.044}, solver="lbfgs", max_iter=1000,
                                          verbose=0)
