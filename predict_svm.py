...
"""
使用支持向量机（SVM）通过交叉验证来优化模型参数，并最终训练和评估模型在数据集上的性能
调整不同的超参数：gamma_scale、degree、正则化惩罚
尝试使用 l2 hinge 进行内核近似
通过分析交叉验证的结果，可以选择最佳的超参数来提高模型的性能。
...
Return
----------
打印并绘制超参数调整的结果（精度、召回率和 f1）
打印训练和测试的精度、精度、召回率、f1 和对核近似的支持
"""
